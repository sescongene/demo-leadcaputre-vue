...
const submitForm = async () => {
  ...
  try {
    const response = await fetch(
      'https://bot.mandalanarratives.com/webhook/2f7f8f15-58e2-4a68-b14f-5fdcf335034c',
      {
        method: 'POST',
        headers: {
...
<template>
  ...
  >
    <!-- Background Decor -->
    <div
      class="absolute -top-40 -right-40 w-96 h-96 bg-purple-600 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"
    ></div>
    <div
      class="absolute -bottom-40 -left-40 w-96 h-96 bg-indigo-600 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000"
    ></div>
    ...
    <div class="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 z-10">
      ...
      <div class="flex flex-col justify-center text-white space-y-6">
        ...
        <p class="text-lg text-slate-300 leading-relaxed max-w-md">
          Join our exclusive network today. Provide your details and we will reach out to schedule a
          free strategy session tailored just for you.
        </p>
      ...
      <!-- Right side form -->
      <div
        class="bg-slate-800/60 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-slate-700/50"
      >
        ...
        <form @submit.prevent="submitForm" class="space-y-5">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              <label for="firstName" class="text-sm font-medium text-slate-300"
              ...
              <input
                ...
                id="firstName"
                type="text"
                class="w-full px-4 py-3 rounded-lg bg-slate-900 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all shadow-inner"
                placeholder="John"
              />
            </div>
            <div class="space-y-1">
              <label for="lastName" class="text-sm font-medium text-slate-300"
          ...
          <div
            v-if="errorMessage"
            class="bg-red-500/10 border border-red-500/50 text-red-400 rounded-lg p-3 text-sm flex items-center shadow-lg"
          >
            <svg
          ...
          <div
            v-if="success"
            class="bg-green-500/10 border border-green-500/50 text-green-400 rounded-lg p-3 text-sm flex items-center shadow-lg"
          >
            <svg
              class="w-5 h-5 mr-2 flex-shrink-0"
              fill="none"
